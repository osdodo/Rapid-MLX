...
import { highlight, type HastNode } from './highlight';
import { parseMarkdown } from './lex';
import { safeHref, safeImageSrc } from './links';
import { segmentLaTeX } from './latex';
 ...
 *
 * Two renderers, chosen per-message by `status`:
 *
 *   Blocks           the settled renderer — highlighting, copy buttons, math
 *   StreamingBlocks  the cheap one — plain code, closed math only
 *
...
export const Blocks = memo(function Blocks({ tokens, streaming, mathRendering }: BlocksProps) {
  return (
    <>
    ...
    case 'blockquote': {
      ...
      return (
        <blockquote className={'md-quote'}>
          <Blocks tokens={quote.tokens} streaming={streaming} mathRendering={mathRendering} />
        </blockquote>
      );
    }
        ...
        >
          {list.items.map((item, index) => (
            <li key={index} className={'md-li'}>
              ...
              <Blocks tokens={item.tokens} streaming={streaming} mathRendering={mathRendering} />
            </li>
          ))}
        </Tag>
      );
...
function CodeBlock({ token, streaming }: { token: Tokens.Code; streaming: boolean }) {
  ...
  return (
    <div className={'md-code-wrap'}>
      {!streaming ? (
        <div className={"border-line-soft font-body flex items-center justify-between gap-2 border-b py-[5px] pr-2 pl-3"}>
          <span className={"font-mono text-faint text-[10.5px] tracking-[0.08em] uppercase"}>{token.lang?.trim().split(/\s+/)[0] ?? ''}</span>
          <button type="button" className={"text-muted hover:bg-line-soft hover:text-fg rounded-sm px-2 py-[3px] text-xs"} onClick={onCopy}>
            {copied ? 'Copied' : 'Copy'}
          </button>
        </div>
      ) : null}
...
function InlineSource({
  ...
  return segments.map((segment, index) => {
    ...
    if (mathRendering === 'source' || streaming) {
      return (
        <code key={index} className={"font-mono text-muted text-[0.88em]"}>
          {segment.latex}
        </code>
      );
    }
...
function InlineToken({
  ...
  switch (token.type) {
    ...
    case 'image': {
      ...
      if (src === null) return <span className={"text-muted italic"}>{image.text}</span>;
...
function MathRun({ latex, display }: { latex: string; display: boolean }) {
  ...
  if (markup === '') {
    return <code className={"font-mono text-muted text-[0.88em]"}>{latex}</code>;
  }
  ...
  return (
    <span
      ...
      // fixtures in Blocks.test.tsx cover the rest, and a Temml version bump
      // must re-run them.

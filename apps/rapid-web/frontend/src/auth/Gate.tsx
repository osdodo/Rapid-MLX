...
import { rememberToken } from './token';
import { Button } from '../ui/primitives/Button';
import { Wordmark } from '../ui/primitives/Wordmark';
...
export function Gate({
  ...
  return (
    <div className="bg-canvas fixed inset-0 z-30 flex items-center justify-center p-6">
      <form className="flex w-[min(320px,100%)] flex-col gap-3" onSubmit={submit}>
        <Wordmark className="mb-1.5 text-center text-[30px]" />
        ...
        <input
          ...
          className="border-line bg-card font-mono focus:border-brand w-full rounded-md border px-3.5 py-3 text-center tracking-[0.02em]"
          type="password"
          value={value}
        ...
        <Button
          type="submit"
          variant="primary"
          className="h-auto rounded-md py-3"
        ...
        <p className="text-danger m-0 min-h-5 text-center text-[13px]" role="alert">
          {error}
        </p>
      </form>
    </div>
